...
- (void)subscribe:(CDVInvokedUrlCommand*)command{
    ...
    NSLog(@"iOS fitMode: %@", fitMode);

    if([fitMode isEqualToString:@"contain"]) {
        [sub setViewScaleBehavior: OTVideoViewScaleBehaviorFit];
    }
...
- (NSMutableDictionary*)createDataFromStream:(OTStream*)stream{
    ...
    if ([stream videoType] == OTStreamVideoTypeScreen) {
        [streamData setObject: @"screen" forKey: @"videoType"];
    } else {
        [streamData setObject: @"camera" forKey: @"videoType"];
    }
    return streamData;
}
